...
import { ToolDefinition } from '@/lib/state';
...
type ToolEditorModalProps = {
  tool: ToolDefinition;
  onClose: () => void;
  onSave: (updatedTool: ToolDefinition) => void;
};
...
export default function ToolEditorModal({
  ...
  useEffect(() => {
    if (tool) {
      // FIX: tool.name, tool.description, tool.parameters are now correctly typed via ToolDefinition
      setName(tool.name);
